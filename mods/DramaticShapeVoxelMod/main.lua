...
mod.content.render_pipelines:register("voxel", {
  ...
  drawWorld = function(ctx)
    ...
    if Voxel3D.beginOverlay() then
      ...
      -- the first-person look stick rides the same overlay, in the same
      -- supersampled pixels; it draws nothing on any other rung
...
-- The language row. Stepping it rewrites every registered label in place
...
do
  ...
  local cycle = ModSetting.cycle
  function langSetting:cycle(game, dir)
    local r = cycle(self, game, dir)
    Lang.set(self:get())
    return r
  end
end
...
local SETTINGS = {
  -- The first-person look preferences. `when` keeps them off the menu on
  -- every other rung: three rows about a stick that is not on screen are
  -- three rows of noise, and the mod manager's page still carries them.
...
mod.hooks:wrap("ui.options.rows", function(next, game, rows)
  ...
  --
  -- A row snapshots its label when it is built, so rewriting the tables only
  -- shows up if the rewrite happened to run first -- which is why switching
  -- the language appeared to do nothing. Doing it here cannot lose that race:
  -- these rows are rebuilt every time the menu is opened, and the value is a
  -- function, so both halves answer in the language that is current at the
  ...
  -- came out of the ROM -- a name, a place, an item -- can be caught by it,
  -- because none of that is in the table.
  for _, row in ipairs(rows) do
    if type(row) == "table" then
      row.label = Lang.t(row.label)
      local inner = row.value
      if type(inner) == "function" then
        row.value = function(...) return Lang.t(inner(...)) end
      end
    end
  end
  return rows
...
-- touch on open screen as a look drag, which swallowed every tap on the
-- handle while this was installed before it.
