...
function BattleScene.letterbox()
  ...
  -- A portrait phone can raise the screen clear of the touch pad, so the
  -- letterbox is not always at the window's centre. Read the lift the
  -- renderer actually applied rather than recomputing it -- recomputing is
  -- how the two drifted apart before, and the panels drawn against these
  -- pins ended up a few hundred pixels from the picture.
  --
  -- Reads 0 whenever a full-window world is on screen, which is every frame
  -- this scene draws; it matters on the frames either side of the mode
  -- switching off.
  local lift = Renderer.appliedLift or 0
  return math.floor((pw - BattleScene.GB_W * s) / 2),
         math.floor((ph - BattleScene.GB_H * s) / 2 - lift),
         s, pw, ph
end
